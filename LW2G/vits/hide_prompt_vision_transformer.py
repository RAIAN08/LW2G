...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, task_id=-1, prompt_id=None, prompt_weight=None, train=False, prompt_momentum=0,
                         cls_features=None,
                         available_mini_model_list=None, enhance_id=None):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            if self.use_g_prompt or self.use_e_prompt:
                ...
                res = self.e_prompt(
                    x, prompt_mask=prompt_mask, prompt_idx=prompt_id, prompt_weight=prompt_weight, 
                    prompt_momentum=prompt_momentum, 
                    available_mini_model_list=available_mini_model_list, training=train, 
                ...
                for i, block in enumerate(self.blocks):
                    x = block(x)
                    feats_l.append(x.detach())
                res = dict()
                res['layer_feats'] = feats_l
    ...
    def forward(self, x, task_id=-1, prompt_id=None, prompt_weight=None, train=False, fc_only=False, prompt_momentum=0, 
                cls_features=None, enhance_id=None, available_mini_model_list=None):
        if fc_only:
            res = dict()
            x = self.mlp(x)
            x = self.fc_norm(x)
            res['logits'] = self.head(x)
            return res
        res = self.forward_features(x, task_id=task_id, prompt_id=prompt_id, prompt_weight=prompt_weight, 
                                    train=train, prompt_momentum=prompt_momentum, enhance_id=enhance_id,
