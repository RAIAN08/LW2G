...
def get_args_parser(subparsers):
    ...
    subparsers.add_argument('--epochs', default=2, type=int) 
    
    
    # subparsers.add_argument('--no_pgp', action="store_true", help='Not activate prompt gradient projection') # 决定是否为最原始的baseline
    subparsers.add_argument('--no_auto', default=0, type=int) 
    subparsers.add_argument('--model_num', default=0, type=int) 

    subparsers.add_argument('--angle_epsilon', default=0.18, type=float) # 
    subparsers.add_argument('--threshold', default=0.95, type=float) #



    subparsers.add_argument('--use_pre_gradient_constraint', default=1, type=int) # 
    subparsers.add_argument('--threshold2', default=0.1, type=float) #

    
    subparsers.add_argument('--use_old_subspace_forward', default=0, type=int) # 
    subparsers.add_argument('--topk_old_subspace', default=1, type=int) # 取多少个
    subparsers.add_argument('--threshold_pretrained', default=0.95, type=float) # 5

    subparsers.add_argument('--dataset_name', default='cifar100', type=str, help='dataset name')
    ...
    subparsers.add_argument('--world_size', default=8, type=int,
                        help='number of distributed processes')
    subparsers.add_argument('--dist_url', default='env://', help='url used to set up distributed training')

    # Continual learning parameters
    subparsers.add_argument('--num_tasks', default=10, type=int, help='number of sequential tasks') # 10
    ...
    subparsers.add_argument('--size', default=10, type=int,) # 10
    subparsers.add_argument('--length', default=20, type=int, ) # 20
    ...
    subparsers.add_argument('--batchwise_prompt', default=False, type=bool) # True
    ...
    subparsers.add_argument('--freeze', default=['blocks', 'patch_embed', 'cls_token', 
                                'norm', 'pos_embed'], nargs='*', type=list, help='freeze part in backbone model')
